...
function updateStorage() {
          for(var x = 0; x < $('.tabela-proizvoda-div[style*="block"] .cekboks-kalkulatora').length; x++) {
              ...
              if($('.tabela-proizvoda-div[style*="block"] .cekboks-kalkulatora:eq(' + x + ')')[0].checked == true) {
                  var key = 'SNG' + $('.tabela-proizvoda-div[style*="block"] .cekboks-kalkulatora:eq(' + x + ')').parents('.spoljni-div').attr('id')
                  ...
                  console.log('2')
                  var key = 'SNG' + $('.tabela-proizvoda-div[style*="block"] .cekboks-kalkulatora:eq(' + x + '):not(:checked)').parents('.spoljni-div').attr('id')
                  var value = '0'
      ...
      function applyStorage() {
          ...
          for (var i = 0; i < localStorage.length; i++) {
              ...
              if(key.indexOf('SNG') > -1) {
                  if(localStorage.getItem(key) == '1') {
                      var query = '.tabela-proizvoda-div[style*="block"] #' + key.split('SNG')[1] + ' .cekboks-kalkulatora'
                      if($(query).length > 0) $(query).click()
                      
                  } else if(localStorage.getItem(key) == '0') {
                      // neaktivni artikli
                  } else if(parseInt(localStorage.getItem(key)) > 1){
                      var query = '.tabela-proizvoda-div[style*="block"] #' + key.split('SNG')[1] + ' .cekboks-kalkulatora'
                      
                      if($(query).length > 0) {
                          $(query).click()
                          var value = parseInt(localStorage.getItem(key)) - 1
                          
                          for(var y = 0; y < value; y++) {
                              console.log('usususu')
                              $('.tabela-proizvoda-div[style*="block"] #' + key.split('SNG')[1] + ' .increase').click()
                          }
                      }
